...
- **NumPy PPO clipped ratio**: `PPOPolicy.update()` now computes `ratio = exp(new_log_prob - old_lp)` and zeros out the advantage when the ratio is already outside `[1-clip_eps, 1+clip_eps]` and would push it further out; without this the update is plain REINFORCE (no clipping) which causes policy collapse on large advantages
- **profit_factor counts all losses**: `compute_metrics()` uses `non_hold[reward < 0]` as `gross_loss`, not just SL trades; TIMEOUT and regime-penalised trades also have negative reward and excluding them caused profit_factor to show inflated values (e.g. millions) when SL trades were absent
- **PyTorch advantage normalization guarded for n=1**: `PPOPolicyTorch.update()` only normalizes advantages when `numel() > 1`; PyTorch `std()` uses ddof=1 and returns NaN for 1-element tensors, which silently corrupts all network weights through backward()
- **`_synthetic_signals()` references `MODEL_NAMES`**: removed duplicate inline `names` list; synthetic fallback now uses the same `MODEL_NAMES` constant as every other code path, so model additions/renames stay consistent
- **Non-ASCII in string literals checked with AST walk**: all module docstrings, argparse descriptions, and file-output strings are now ASCII-only; grep-for-print is insufficient -- AST walk over `ast.Constant` nodes catches all runtime-visible strings
